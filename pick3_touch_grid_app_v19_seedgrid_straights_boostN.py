...
def build_seed_grid(
    ...
    - The *seed digit* for each position is ALWAYS included, and shown first in that column.
    - The *mirror digit* for each position is ALWAYS included (shown second when different).
    - The remaining slots are filled by the best-scoring candidates from neighbors (+/-1),
      and optional position-followers.

    This keeps the grid stable (3x4) while still letting the data pick the extra digits.
    ...
    for pos in range(3):
        ...
        cand_optional = []
        for d in [n_plus, n_minus] + tf_digits:
            if d not in required and d not in cand_optional:
        ...
        slots_left = max(0, int(rows) - len(required))
        chosen = required.copy()
        chosen_scores: dict[int, float] = {}

        # record required scores too
        for d in required:
            comp = {
                "carry": float(w_carry if d == s else 0.0),
                "mirror": float(w_mirror if d == m else 0.0),
        ...
        if len(chosen) < rows:
            ...
            for dd, _ in due_items:
                ...
                if di not in chosen:
                    chosen.append(di)
                    # drought fill gets a tiny score just for display ordering
                    chosen_scores[di] = -9999.0
                if len(chosen) >= rows:
                    break

        # final display order:
        #   seed first, mirror second (if different), remaining by score desc
        head: list[int] = [s]
        if m != s:
            head.append(m)
        tail = [d for d in chosen if d not in head]
        tail_sorted = sorted(tail, key=lambda d: (-chosen_scores.get(d, 0.0), d))
        final = head + tail_sorted
        final = final[: int(rows)]
        ...
        dbg["per_pos"].append(
            {
                ...
                "seed_digit": str(s),
                "mirror_digit": str(m),
                "neighbors": [str(n_minus), str(n_plus)],
...
def rank_candidates(
    ...
    mirror_digits = {str(MIRROR[int(d)]) for d in seed_digits}
...
ranked = rank_candidates(
    ...
    keep_top_n=float(keep_top_n) if keep_top_n < 100 else None,
)
